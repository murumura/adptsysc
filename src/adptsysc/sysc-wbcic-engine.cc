...
template <typename E>
std::unique_ptr<OutputFile<E>> open_export(Context<E>& ctx, const std::filesystem::path& path) {
...
make_test_input(const std::string& name) {
  using T = typename E::Eval_T;
  const std::size_t count = name == "impulse" ? 256 : name == "constant" ? 192 :
                            name == "multitone" ? 320 : 384;
  std::vector<T> data(count, T{});
  std::mt19937 rng(0x5a17u);
  std::uniform_int_distribution<int> distribution(-6, 6);
  for (std::size_t n = 0; n < count; ++n) {
    if (name == "impulse") data[n] = n == 0 ? T(0.5) : T(0);
    else if (name == "constant") data[n] = T(0.125);
    else if (name == "multitone") {
      const double pi = std::acos(-1.0);
      data[n] = static_cast<T>(0.125 * std::sin(2 * pi * n / 64.0) +
                               0.0625 * std::cos(2 * pi * n / 37.0));
...
template <typename E>
void run_case(Context<E>& ctx, const std::filesystem::path& directory, const std::string& name) {
  using Eval_T = typename E::Eval_T;
  using Engine = WbcicEngine<E>;
  using Fxpt_T = typename E::Fxpt_T;
  const auto samples = make_test_input<E>(name);

  auto math = Engine::create(ctx, "wbcic_math_" + name);
  auto fixed = Engine::create(ctx, "wbcic_fixed_" + name);
  ...
  if (config.enabled) fixed->set_quant_config(config);

  const auto prefix = directory / ("wbcic_" + name);
  std::unique_ptr<TraceFile<E>> trace;
  std::unique_ptr<TraceFile<E>> fixed_trace;
  if (ctx.arg.signal_trace) {
    const auto path = name == "impulse" && !ctx.arg.signal_trace_file.empty()
      ? std::filesystem::path(ctx.arg.signal_trace_file) : std::filesystem::path(prefix.string() + ".trace");
    if (!path.parent_path().empty()) std::filesystem::create_directories(path.parent_path());
  ...
  if (ctx.arg.sv_trace) {
    ...
    input_mem = open_export(ctx, mem_dir / ("wbcic_" + name + "_input.mem"));
    golden_mem = open_export(ctx, mem_dir / ("wbcic_" + name + "_golden.mem"));
  }
  ...
  for (std::size_t n = 0; n < samples.size(); ++n) {
    if (n % 23 == 7) {
      if (math->step(Eval_T(0.25), false).output_valid || fir.step(Eval_T(0.25), false).output_valid ||
          (config.enabled && fixed->step(Eval_T(0.25), false).output_valid)) {
        throw std::runtime_error(name + ": stall produced an output");
      }
    ...
    const auto actual = math->step(sample);
    const auto expected = fir.step(sample);
    if (trace) {
      trace->write_line("call=" + std::to_string(call_index) + " accepted=1 " +
                        trace_line(math->get_last_trace(), "math"));
    }
    ++call_index;
    if (actual.output_valid != expected.output_valid ||
        (actual.output_valid && (actual.input_index != expected.input_index ||
                                 actual.output_index != expected.output_index))) {
      throw std::runtime_error(name + ": math/FIR phase or output-index mismatch at n=" + std::to_string(n));
    }

    typename Engine::StepResult fx_result{};
    typename WidebandCicFirRef<E>::StepResult fx_expected{};
    if (config.enabled) {
      const Eval_T q_input = config.quantize_input
        ? static_cast<Eval_T>(wbcic_detail::quantize_format(sc_dt::sc_fxval(static_cast<double>(sample)),
            E::input_word_bits, E::input_integer_bits, config.rounding).to_double()) : sample;
      fx_expected = fixed_fir.step(q_input);
      fx_result = fixed->step(sample);
      if (fixed_trace) {
        fixed_trace->write_line("call=" + std::to_string(call_index - 1) + " accepted=1 " +
                                trace_line(fixed->get_last_trace(), "fixed"));
      }
      if (fx_result.output_valid != actual.output_valid || fx_result.output_valid != fx_expected.output_valid ||
        ...
        throw std::runtime_error(name + ": fixed/FIR output-valid or phase mismatch");
      }
    }
    ...
    if (!std::isfinite(static_cast<double>(error)) || error > 2e-6L) {
      throw std::runtime_error(name + ": math/FIR mismatch at n=" + std::to_string(n));
    }
    if (config.enabled) {
      ...
      if (!std::isfinite(static_cast<double>(fixed_error)) || fixed_error > ctx.arg.fixedpoint_tol + 1e-12) {
        throw std::runtime_error(name + ": fixed/FIR error exceeds tolerance at n=" + std::to_string(n));
      }
    }
    if (golden_mem) {
      if (!mem_expected.output_valid || mem_expected.input_index != expected.input_index) {
        throw std::runtime_error(name + ": MEM/FIR phase mismatch");
      }
      // Reuse the arch's SystemC fixed-point format + existing IO export helper.
      write_syscfx_hexword<E>(*golden_mem, mem_expected.output);
    }
    if (ctx.arg.trace_enabled && outputs <= 12) {
      Out(ctx) << "[WBCIC " << name << "] n=" << n << " y=" << actual.output
               << " fir=" << expected.output << " err=" << static_cast<double>(error) << "\n";
    }
  }

  if (outputs != (samples.size() + E::m - 1) / E::m ||
      math->get_input_count() != samples.size() || math->get_output_count() != outputs ||
      (config.enabled && fixed->get_output_count() != outputs)) {
  ...
  Out(ctx) << "[WBCIC] PASS " << name << " accepted=" << samples.size() << " outputs=" << outputs
...
bool WbcicEngine<E>::run_testbench(Context<E>& ctx) {
  try {
    ...
    for (const std::string name : {"impulse", "constant", "multitone", "random"}) {
      wbcic_detail::run_case<E>(ctx, output_dir, name);
    }
